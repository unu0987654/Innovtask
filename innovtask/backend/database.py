...
class _PgCursorWrap:
    ...
    def execute(self, sql, params=()):
        ...
        if params:
            ...
            # Sin parametros: ejecutar sin segundo argumento para que psycopg2
            # NO intente formatear el SQL (evita 'tuple index out of range'
            # cuando el texto contiene % u otros caracteres).
...
def init_db():
    ...
    cur.execute(f"""
        CREATE TABLE IF NOT EXISTS users(
            ...
            pass_hash TEXT,
            color TEXT,
            google_token TEXT,          -- refresh_token de Google (cifrado), si conecto su calendario
            created_at TEXT
        )""")
    cur.execute(f"""
    ...
    # Migracion para google_token en users (bases ya existentes).
    # En Postgres usamos IF NOT EXISTS para que sea seguro y no aborte nada.
    try:
        if USE_PG:
            cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS google_token TEXT")
        else:
            try:
                cur.execute("ALTER TABLE users ADD COLUMN google_token TEXT")
            except Exception:
                pass
        c.commit()
    except Exception:
...
def _seed_users(c):
    ...
    if USE_PG:
        # Ejecucion nativa en Postgres (sin el wrapper de '?') para evitar
        # cualquier ambiguedad en la traduccion de marcadores de posicion.
        raw = c._conn.cursor()
        raw.executemany(
...
def sync_seed_roles():
    """Corrige el rol de los usuarios ya existentes segun SEED_USERS.
    Util cuando un usuario quedo con un rol viejo (ej. German como coordinador
    en vez de procesos). No borra nada, solo actualiza el campo role."""
    c = _conn()
    updated = []
    for name, email, role in SEED_USERS:
        idx = _email_index(email)
        cur = c.execute("SELECT role FROM users WHERE email_idx=?", (idx,))
        row = cur.fetchone()
        if row and row["role"] != role:
            c.execute("UPDATE users SET role=? WHERE email_idx=?", (role, idx))
            updated.append(f"{name}: {row['role']} -> {role}")
    c.commit()
    c.close()
    return updated
...
# ---------------------------------------------------------------------------
# Google Calendar: guardar/leer el token de cada usuario (cifrado)
# ---------------------------------------------------------------------------
def save_google_token(email, refresh_token):
    """Guarda (cifrado) el refresh_token de Google del usuario."""
    c = _conn()
    enc = encrypt_field(refresh_token) if refresh_token else None
    c.execute("UPDATE users SET google_token=? WHERE email_idx=?",
              (enc, _email_index(email)))
    c.commit()
    c.close()


def get_google_token(email):
    """Devuelve el refresh_token descifrado, o None si no conecto Google."""
    c = _conn()
    row = c.execute("SELECT google_token FROM users WHERE email_idx=?",
                    (_email_index(email),)).fetchone()
    c.close()
    if not row:
        return None
    tok = row["google_token"]
    if not tok:
        return None
    try:
        return decrypt_field(tok)
    except Exception:
        return None


def has_google_connected(email):
    return get_google_token(email) is not None
